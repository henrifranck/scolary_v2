...
export type AcademicYearListQuery = Record<string, string | number | boolean | undefined>;

const academicYearsKey = ['academicYears'] as const;

const queryKeys = {
  academicYears: (query?: AcademicYearListQuery): QueryKey => (query ? [...academicYearsKey, query] : academicYearsKey),
...
export const fetchAcademicYears = async (query?: AcademicYearListQuery): Promise<{ data: AcademicYear[]; count?: number }> =>
  normalizeList(await apiRequest<ApiListResponse<AcademicYear> | AcademicYear[]>('/academic_years/', { query }));

export const fetchAcademicYear = (id: number): Promise<AcademicYear> => apiRequest<AcademicYear>(`/academic_years/${id}/`);

export const createAcademicYear = (payload: AcademicYearPayload): Promise<AcademicYear> =>
  apiRequest<AcademicYear>('/academic_years/', {
    method: 'POST',
    json: payload
  });

export const updateAcademicYear = (id: number, payload: AcademicYearPayload): Promise<AcademicYear> =>
...
export const useUpdateAcademicYear = () => {
  ...
  return useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: AcademicYearPayload }) => updateAcademicYear(id, payload),
