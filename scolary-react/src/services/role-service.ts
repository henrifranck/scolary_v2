...
import { Role, RoleListQuery, RolePayload } from "@/models/role";

const rolesKey = ["roles"] as const;

const queryKeys = {
  roles: (query?: RoleListQuery): QueryKey =>
    query ? [...rolesKey, query] : rolesKey,
...
export const fetchRoles = async (
  ...
  normalizeList(
    await apiRequest<ApiListResponse<Role> | Role[]>("/roles/", {
      query: withDefaultRelation(query)
    })
  );

export const fetchRole = (id: number): Promise<Role> =>
  apiRequest<Role>(`/roles/${id}/`, { query: withDefaultRelation() });
