...
const mockSteps = [
  { id: "profile", label: "Update student profile", completed: true },
  { id: "documents", label: "Collect validation documents", completed: false },
  { id: "payment", label: "Confirm payment of fees", completed: false }
];
...
export const InscriptionPage = () => {
  ...
  const defaultJourneyId =
    journeyOptions.find((journey) => journey.mentionId === defaultMentionId)
      ?.id ?? "";
  ...
  const availableJourneys = useMemo(
    () =>
      journeyOptions.filter(
        (journey) => journey.mentionId === filters.mentionId
      ),
    [filters.mentionId]
  );
